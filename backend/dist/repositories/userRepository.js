...
class UserRepository {
    ...
    static async getStudentProfile(userId) {
        const { data, error } = await this.getClient()
            ...
            .maybeSingle();
        if (error)
            throw new Error(`Student profile error: ${error.message}`);
        return data;
    }
    static async createStudentProfile(profile) {
        const { data, error } = await this.getClient()
            .from('student_profiles')
            .insert([profile])
            .select('*')
            .single();
        if (error)
            throw new Error(`Create student profile error: ${error.message}`);
        return data;
    }
    static async updateStudentProfile(userId, updates) {
        const { data, error } = await this.getClient()
            ...
            .single();
        if (error)
            throw new Error(`Update student profile error: ${error.message}`);
        return data;
    }
    static async getDriverProfile(userId) {
        const { data, error } = await this.getClient()
            ...
            .maybeSingle();
        if (error)
            throw new Error(`Driver profile error: ${error.message}`);
        return data;
    }
    static async createDriverProfile(profile) {
        const { data, error } = await this.getClient()
            .from('driver_profiles')
            .insert([profile])
            .select('*')
            .single();
        if (error)
            throw new Error(`Create driver profile error: ${error.message}`);
        return data;
    }
    static async updateDriverProfile(userId, updates) {
        const { data, error } = await this.getClient()
            .from('driver_profiles')
            .update({ ...updates, updated_at: new Date().toISOString() })
            .eq('id', userId)
            ...
            .single();
        if (error)
            throw new Error(`Update driver profile error: ${error.message}`);
        return data;
    }
    static async getAdminProfile(userId) {
        const { data, error } = await this.getClient()
            ...
            .maybeSingle();
        if (error)
            throw new Error(`Admin profile error: ${error.message}`);
        return data;
    }
    static async createAdminProfile(profile) {
        const { data, error } = await this.getClient()
            .from('admin_profiles')
            .insert([profile])
            .select('*')
            .single();
        if (error)
            throw new Error(`Create admin profile error: ${error.message}`);
        return data;
    }
    static async updateAdminProfile(userId, updates) {
        const { data, error } = await this.getClient()
            ...
            .single();
        if (error)
            throw new Error(`Update admin profile error: ${error.message}`);
        return data;
    }
    static async upsertAdminProfile(profile) {
        const { data, error } = await this.getClient()
            .from('admin_profiles')
            ...
            .single();
        if (error)
            throw new Error(`Upsert admin profile error: ${error.message}`);
        return data;
    }
    static async getAllAdmins() {
        ...
        if (error)
            throw new Error(`Fetch admins error: ${error.message}`);
        return (data || []).map((p) => ({
            ...p.user,
            profile: p,
        }));
    ...
    static async getAllStudents(collegeId) {
        ...
        if (error)
            throw new Error(`Fetch students error: ${error.message}`);
        return (data || []).map((p) => ({
            ...p.user,
            profile: p,
            college: p.college,
    ...
    static async getAllDrivers(collegeId) {
        let query = this.getClient()
            .from('driver_profiles')
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Fetch drivers error: ${error.message}`);
        return (data || []).map((p) => ({
            ...p.user,
            profile: p,
            college: p.college,
    ...
    static async deleteStudent(userId) {
        ...
        if (bookings && bookings.length > 0) {
            const err = new Error(`Cannot delete student: ${bookings.length} active confirmed booking(s) exist. Please cancel bookings or deactivate the account instead.`);
            err.statusCode = 409;
        ...
        if (subs && subs.length > 0) {
            const err = new Error('Cannot delete student with active subscription. Please expire or cancel the subscription first, or deactivate the user.');
            err.statusCode = 409;
            err.code = 'ACTIVE_RELATIONSHIP_EXISTS';
            throw err;
        }
