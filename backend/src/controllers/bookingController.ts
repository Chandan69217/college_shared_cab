...
import { createBookingSchema, cancelBookingSchema, checkAvailabilitySchema } from '../validators/schemas';
import { sendSuccess } from '../utils/response';

export class BookingController {
  public static async checkAvailability(req: Request, res: Response, next: NextFunction) {
    try {
      const studentId = req.user!.userId;
      const validated = checkAvailabilitySchema.parse(req.body);
      const result = await BookingService.checkAvailability(
        studentId,
        validated.route_id,
        validated.pickup_point_id,
        validated.drop_point_id,
        validated.trip_id,
        validated.trip_type
      );
      sendSuccess(res, result.message, result, result.available ? 200 : 200);
    } catch (err) {
      next(err);
    }
  }
