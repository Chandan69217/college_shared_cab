...
export class RouteRepository {
  ...
  public static async findAll(collegeId?: string): Promise<Route[]> {
    ...
    if (error) throw new Error(`Fetch routes error: ${error.message}`);
    
    return ((data || []) as any[]).map((r) => ({
      ...r,
      stops: (r.route_pickup_points || []).sort((a: any, b: any) => a.sequence_order - b.sequence_order),
    })) as Route[];
  }
  ...
  public static async findById(id: string): Promise<Route | null> {
    ...
    if (error) throw new Error(`Fetch route error: ${error.message}`);
    if (!data) return null;

    return {
      ...data,
      stops: ((data as any).route_pickup_points || []).sort((a: any, b: any) => a.sequence_order - b.sequence_order),
    } as Route;
  }

  public static async create(route: Partial<Route>, stops?: any[]): Promise<Route> {
    const { data, error } = await this.getClient()
      .from('routes')
      .insert([{
    ...
    if (error) throw new Error(`Create route error: ${error.message}`);
  ...
  public static async update(id: string, updates: Partial<Route>, stops?: any[]): Promise<Route> {
    const { data, error } = await this.getClient()
      .from('routes')
    ...
    if (error) throw new Error(`Update route error: ${error.message}`);
  ...
  public static async delete(id: string): Promise<void> {
    ...
    if (bookings && bookings.length > 0) {
      const err: any = new Error(
        `Cannot delete route: ${bookings.length} active booking(s) exist on this route. Please reassign or cancel bookings, or set route status to inactive.`
      );
      err.statusCode = 409;
    ...
    if (trips && trips.length > 0) {
      const err: any = new Error(
        `Cannot delete route: ${trips.length} active or scheduled trip(s) depend on this route. Reassign trips or mark route inactive instead.`
      );
      err.statusCode = 409;
      err.code = 'ACTIVE_RELATIONSHIP_EXISTS';
      throw err;
    ...
    if (error) throw new Error(`Delete route error: ${error.message}`);
  }
}
