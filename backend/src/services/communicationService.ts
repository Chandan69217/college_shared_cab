...
export class CommunicationService {
   ...
   */
  private static getEmailTransporter(): Transporter {
...
<body>
  <div class="card">
    ...
    <div class="security-notice">
      ...
      logger.info(`[EMAIL DISPATCH] Successfully sent OTP ${otp} to email: ${toEmail}`);
      return true;
    } catch (err: any) {
      logger.error(`[EMAIL DISPATCH FAILED] Error sending OTP to ${toEmail}: ${err.message}`);
      return false;
    }
  }
  ...
  public static async sendOtpSms(
    ...
    if (ENV.SMS_GATEWAY_PROVIDER === 'FAST2SMS' && ENV.FAST2SMS_API_KEY) {
      return new Promise((resolve) => {
        ...
        const req = https.request({
          ...
          res.on('end', () => {
            logger.info(`[FAST2SMS RESPONSE] Status: ${res.statusCode} Body: ${body}`);
            resolve(res.statusCode === 200);
          });
        });

        req.on('error', (e) => {
          logger.error(`[FAST2SMS ERROR]: ${e.message}`);
          resolve(false);
        });
    ...
    // 2. Twilio Provider
    if (ENV.SMS_GATEWAY_PROVIDER === 'TWILIO' && ENV.TWILIO_ACCOUNT_SID && ENV.TWILIO_AUTH_TOKEN) {
      return new Promise((resolve) => {
        ...
        const req = https.request({
          ...
          res.on('data', (d) => body += d);
          res.on('end', () => {
            logger.info(`[TWILIO SMS RESPONSE] Status: ${res.statusCode}`);
            resolve(res.statusCode === 200 || res.statusCode === 201);
          });
        });

        req.on('error', (e) => {
          logger.error(`[TWILIO ERROR]: ${e.message}`);
          resolve(false);
        });
    ...
    logger.info(`[SMS DISPATCH] Recipient: +91-${cleanPhone} | Message: "${message}"`);
    return true;
  }
}
